...
class TodoService {
  ...
  async getAllTodos(homeId: string): Promise<TodoItem[]> {
    ...
    const data = await fileSystemService.readFile<TodosData>(TODOS_FILE, homeId);
  ...
  async getTodoById(id: string, homeId: string): Promise<TodoItem | null> {
    ...
    const todos = await this.getAllTodos(homeId);
   ...
   * Create a new todo
   */
  async createTodo(text: string, homeId: string, note?: string, categoryId?: string): Promise<TodoItem | null> {
    try {
      if (!homeId) {
      ...
      const todos = await this.getAllTodosForSync(homeId);
      ...
      todos.push(newTodo);
      const success = await fileSystemService.writeFile<TodosData>(TODOS_FILE, { todos }, homeId);
  ...
  async updateTodo(
    ...
    try {
      ...
      const success = await fileSystemService.writeFile<TodosData>(TODOS_FILE, { ...data, todos }, homeId);
  ...
  async syncTodos(
    ...
    try {
      ...
      const freshData = await fileSystemService.readFile<TodosData>(TODOS_FILE, homeId);
      if (freshData?.todos) {
      ...
      await fileSystemService.writeFile<TodosData>(TODOS_FILE, {
        todos,
