...
class CategoryService {
  ...
  async getAllCategories(homeId: string): Promise<Category[]> {
    ...
    const data = await fileSystemService.readFile<CategoriesData>(CATEGORIES_FILE, homeId);
   ...
   */
  async createCategory(category: Omit<Category, 'id' | 'version' | 'clientUpdatedAt' | 'homeId'>, homeId: string): Promise<Category | null> {
    try {
      if (!homeId) {
      ...
      const success = await fileSystemService.writeFile<CategoriesData>(CATEGORIES_FILE, { ...data, categories }, homeId);
  ...
  async deleteCategory(id: string, homeId: string): Promise<boolean> {
    try {
      ...
      return await fileSystemService.writeFile<CategoriesData>(CATEGORIES_FILE, { ...data, categories }, homeId);
  ...
  async syncCategories(
    ...
    try {
      ...
      await fileSystemService.writeFile<CategoriesData>(CATEGORIES_FILE, {
        categories,
